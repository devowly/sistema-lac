...
List.prototype.fetch = function(req, res, context) {
  ...
  var orderParam = (this.resource.order ? this.resource.order.param || 'order' : 'order');
  if ((_.has(req.query, sortParam) || _.has(this.resource.sort, 'default')) && !_.has(req.query, orderParam)) {
    var order = [];
    var columnNames = [];
  ...
  } else if ((_.has(req.query, sortParam) || _.has(this.resource.sort, 'default')) && _.has(req.query, orderParam)) {
    // Se houver o parametro order nós vamos fazer o sorteio aqui.
    var order = [];
    var columnNames = [];
    var sortQuery = req.query[sortParam] || this.resource.sort.default || '';
    var orderQuery = req.query[orderParam];
    var sortColumns = sortQuery.split(',');
    sortColumns.forEach(function(sortColumn) {
      var actualName = sortColumn;
      if (sortColumn.indexOf('-') === 0) {
      ...
      if (orderQuery === 'DESC' || orderQuery === '1') {
        ...
        order.push([sortColumn, 'ASC']);
         ...
         throw new errors.BadRequestError('Ordem de sorteio incorreto.');
      }
    });
    var allowedColumns = this.resource.sort.attributes || Object.keys(model.rawAttributes);
    var disallowedColumns = _.difference(columnNames, allowedColumns);
